...
public class ReceiverTestHub : Hub<IReceiver>, IReceiverTestHub
{
    ...
    public async Task Start()
    {
        ...
        {
            await this.Clients.Caller.ReceiveCustomMessage(new UserDefinedType() { Guid = Guid.Parse(_guids[i]), DateTime = DateTime.SpecifyKind(DateTime.Parse(_dateTimes[i]), DateTimeKind.Utc) });
        }
    }
}
