...
const oneReceiverMethodPerSubscription = async () => {
    ...
    const receiver1: Partial<IReceiver> = {
        receiveMessage: (message: string, value: number): Promise<void> => {
            receiveMessageList.push([message, value]);
            return Promise.resolve();
        }
    }
    const receiver2: Partial<IReceiver> = {
        nofity: (): Promise<void> => {
            notifyCallCount += 1;
            return Promise.resolve();
        }
    }
    const receiver3: Partial<IReceiver> = {
    ...
    const subscription1 = getReceiverRegister("IReceiver")
        .register(connection, receiver1 as IReceiver);
    const subscription2 = getReceiverRegister("IReceiver")
        .register(connection, receiver2 as IReceiver);
    const subscription3 = getReceiverRegister("IReceiver")
        .register(connection, receiver3 as IReceiver);
...
const oneReceiverMethodPerSubscriptionWithDisposing = async () => {
    const connection = new HubConnectionBuilder()
        .withUrl("http://localhost:5000/realtime/receivertesthub")
        .build();
    ...
    expect(notifyCallCount).toEqual(17*2);

    subscription2.dispose();
    await connection.stop()
}

const oonlyOneReceiverMethod = async () => {
    const connection = new HubConnectionBuilder()
        .withUrl("http://localhost:5000/realtime/receivertesthub")
        .build();
    ...
    const receiver: Partial<IReceiver> = {
        nofity: (): Promise<void> => {
            notifyCallCount += 1;
            return Promise.resolve();
        }
    ...
    const subscription = getReceiverRegister("IReceiver")
        .register(connection, receiver as IReceiver);
...
test('receiver.parcial.test.oneReceiverMethodPerSubscription', oneReceiverMethodPerSubscription);
test('receiver.parcial.test.oneReceiverMethodPerSubscriptionWithDisposing', oneReceiverMethodPerSubscriptionWithDisposing);
test('receiver.parcial.test.oonlyOneReceiverMethod', oonlyOneReceiverMethod);
